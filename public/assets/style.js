  ...
  if (e.target.classList.contains("ajax")) {
    ...
    switch (e.target.form.id) {
      ...
      case "create-delivery":
        var quotationItemsCounts =
          $(".delivery-items").find(".delivery-item").length;
        if (quotationItemsCounts == 0) {
          $("#voyager-loader").hide();
          e.target.disabled = false;
          e.target.innerHTML = button;

          toastr.error(
            "No delivery items present, please add items to project."
          );
        ...
        break;
      case "create-receipt":
        var receiptItems = $(".cash-receipt-item").length;
        ...
        if (receiptItems == 0) {
          ...
          toastr.error("No invoices found for selected customer.");

          return false;
        }
        break;
      case "create-costume-measurement":
        var itemCount = $(".costume-measurement-row").length;
        var newItemCount = $(".new-measurement").length;
        ...
        if (itemCount == 0) {
          ...
          toastr.error("Please add some measurements.");

          return false;
        }

        formData.append("measurement_count", itemCount);
        formData.append("new_measurement_count", newItemCount);
        break;
      case "edit-quotation":
        var itemCount = $(".delete-quotation-item-row").length;
        if (itemCount == 0) {
          $("#voyager-loader").hide();
          e.target.disabled = false;
          e.target.innerHTML = button;

          toastr.error("Please add some items.");

          return false;
        }
        break;
      case "suppliersForm":
        var itemCount = $(".delete-info").length;
        if (itemCount == 0) {
          $("#voyager-loader").hide();
          e.target.disabled = false;
          e.target.innerHTML = button;

          toastr.error("Please add some contact info.");
      ...
      response.json().then((data) => {
        ...
        if (data.response == "errors") {
          ...
          switch (data.title) {
            ...
            case "details":
            case "procurement":
            case "internal-approval-routing":
            case "timeline":
            case "man-power-costing":
            case "template-management":
            case "item-transfer-tracking":
            case "costume-management":
            case "mascot-store":
            case "project-estimate":
            case "suppliers":
            case "props-management":
            case "costume-measurement":
            case "costume-quality-checks":
            case "holidays":
            case "invoice-management":
            case "delivery-management":
            case "cash-receipts":
            case "costumes-measurements":
            case "gst-numbers":
            case "made-to-measure":
            case "mascot-measurements":
            case "bill-of-materials":
            case "mascot-quality-checks":
            case "package-list":
            case "assign-single-employee-save":
            case "assign-multiple-employee":
            case "goods":
            case "group":
            case "over-head":
            case "supplier":
            ...
            case "mascot-update":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
                $("#partialMascotModal").modal("toggle");
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
                $("#partialMascotModal").modal("toggle");
              }
            ...
            case "shipping-addresses":
              ...
              $("#tblShippingAddressList").DataTable().ajax.reload();
              $("#shipping-addresses-form").trigger("reset");
              $("#single-shipping-aadress-delete-method").val("DELETE");
              $("#bulk-shipping-address-delete-method").val("DELETE");
              toastr.success(data.message);
              break;
            case "safety-stock-levels":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
            ...
            case "sam":
              if (data.id !== undefined) {
                let option =
                  ...
                  data.task_name +
                  "</option>";
                $("#mytask").append(option);
                $("#task_add").trigger("reset");
                $("#add-sam-task").modal("hide");
                toastr.success(data.message);
              } else {
                if (data.redirect_url.includes("?")) {
                  window.location.href =
            ...
            case "currencies":
              ...
              break;
            case "currencies-partials":
              $("#multiCurrencyConverterModal").modal("hide");
              toastr.success(data.message);
              break;
            case "goods":
              if (data.redirect_url.includes("?")) {
            ...
            case "fabric-proposals":
              ...
              break;
            case "fabric-proposals-part-update":
              // toastr.success(data.message);
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
            ...
            case "supplier-ratings":
              toastr.success(data.message);
              break;
            case "purchase-sheet-items":
              toastr.success(data.message);
              break;
            case "project-quotations":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
            ...
            case "fabric-proposals-update":
              ...
              break;
            case "fabric-proposals-part-add":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
            ...
            case "inventory-update":
              if (data.id !== undefined) {
                let option =
                  ...
                  data.item_name +
                  "</option>";
                $("#inventory_id").append(option);
                $("#addInventroryPartialsModalForm").trigger("reset");
                $("#addInventroryPartialsModal").modal("hide");
                toastr.success(data.message);
              } else {
                if (data.redirect_url.includes("?")) {
                  window.location.href =
              ...
              break;
            case "project-quotations-mail":
            case "project-invoices-mail":
              $("#preview-email-modal").modal("hide");
              toastr.success(data.message);
              break;
            case "project-invoices-approval":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
              ...
              break;
            case "project-quotations-terms":
            case "project-invoice-terms":
              $("#editTermsConditionsModal").modal("hide");
              toastr.success(data.message);
              break;
            case "purchase-sheet-supplier":
              let supplierOption =
                "<option value='" +
                data.supplier_id +
                "' selected>" +
              ...
              $('select[name="supplier_id[]"]').append(supplierOption);
              initSelect2();
              toastr.success(data.message);
              break;
            case "inventory-suppliers":
              $("#current_supplier_id").val(data.supplier_id);
              $("#adminSupplierTable")
                .DataTable()
                .ajax.url(data.datatable_reload_url)
                .load();
              $("#suppliersForm").trigger("reset");
              toastr.success(data.message);
              break;
            case "shipping-locations":
              if (data.redirect_url.includes("?")) {
                window.location.href = data.redirect_url + "&q=" + data.message;
              } else {
                window.location.href = data.redirect_url + "?q=" + data.message;
            ...
            case "brands":
              ...
              $("#tblBrandsList").DataTable().ajax.reload();
              $("#brand_add_form").trigger("reset");
              $("#brand_import_form").trigger("reset");
              toastr.success(data.message);
              break;
            case "attributes":
              if (data.redirect_url.includes("?")) {
            ...
            case "global-variables":
              if (data.id !== undefined) {
                $("#add-mascot-modal").modal("hide");
                $("#categoryPartialsEditAddForm").modal("hide");
                ...
                if ($("#mascot_category").length > 0 && data.status == "1") {
                  $("#mascot_category").append(option).trigger("change");
                  $("#categoryPartialsEditAddForm").trigger("reset");
                }
              } else {
                if (data.redirect_url.includes("?")) {
                  window.location.href =
            ...
            case "companies":
              $("#add_modal").modal("hide");
              $("#tblAttrValuesList").DataTable().ajax.reload();
              $("#attribute-value-form").trigger("reset");
              $("#single-delete-method").val("DELETE");
              $("#bulk-delete-method").val("DELETE");
              toastr.success(data.message);
              break;
            case "sourcing-request-comment":
              document.getElementById("attachments_file").innerHTML = "";
              let request_id = data.request_id;
              let commenthtml = data.html;
              let actionUrl =
                baseUrl +
                "/admin/sourcing-request/comments?request_id=" +
                request_id;
              $("#request_comments").html("");
              $.get(actionUrl, function (response) {
                $("#request_comments").append(response.comment);
              });
              $("#sourcing_comment").trigger("reset");
              break;
            case "receive-supplier-add":
              var html = "";
              $("#basicExampleModal").modal("hide");
              document.getElementById(
                "supplier_drop" + data.data_id
              ).innerHTML = "";
              for (var i = data.supplier.length - 1; i >= 0; i--) {
                if (data.supplier[i].id == data.created) {
                  html +=
                    "<option value=" +
                    data.supplier[i].id +
                    ' selected="selected">' +
                    data.supplier[i].name +
                    "</option>";
                } else {
                  html +=
                    "<option value=" +
            ...
            case "finance-management":
              ...
              $("#adminEmployeeSalaryTable").DataTable().ajax.reload();
              break;
            case "project-cost":
              $("#projectCostingModal").modal("hide");
              $("#adminFinanceProjectTable").DataTable().ajax.reload();
              break;
            case "add-inventory":
              var inventoryModal = $("#addInventoryModal"),
                transferItemIndex = inventoryModal
                  .find('input[name="tranfer_item_index"]')
                  .val(),
                quantity = $("#addInventoryModal")
                  .find('input[name="quantity"]')
                  .val();

              $('input[name="quantity$' + transferItemIndex + '"]').val(
                quantity
              );
              inventoryModal.modal("hide");
              break;
            case "send_employee_invitation":
              $("#invitead").modal("hide");
              toastr.success(data.message);
              break;
            case "costume-measurements":
              var measurementModal = $("#costumeMeasurementsModal");
              measurementModal.find(".costume-measurements-add-form").hide();
              measurementModal.find(".costume-measurements-edit-form").hide();
              measurementModal.find(".costume-measurements-table").show();
              measurementModal.find(".modal-footer").show();

              $("#costume-measurements-table").load(
                document.URL + " #costume-measurements-table"
              );
              break;
            case "add-inventory-costume":
              var addInventoryModalCostume = $("#addInventoryModalCostume"),
                costumeIndex = addInventoryModalCostume
                  .find('input[name="costume_index"]')
                  .val(),
                costumePartIndex = addInventoryModalCostume
                  .find('input[name="costume_part_index"]')
                  .val(),
                costumePartInventoryIndex = addInventoryModalCostume
                  .find('input[name="costume_part_inventory_index"]')
                  .val(),
                quantity = addInventoryModalCostume
                  .find('input[name="quantity"]')
                  .val();
              ...
              addInventoryModalCostume.modal("hide");
              break;
            case "inventory-import":
              $("#export-inventory").modal("hide");
              $("#adminInventoriesTable").DataTable().ajax.reload();
              toastr.success(data.message);
              location.reload();
              break;
            ...
            case "supplier-import":
              $("#import-supplier").modal("hide");
              $("#adminSupplierTable").DataTable().ajax.reload();
              toastr.success(data.message);
              break;
            case "project-ticket-store":
              $("#sendMessageModal").modal("hide");
              $("#projectTicketStore").trigger("reset");
              toastr.success(data.message);
              break;
            case "costume-quality-checks-update":
              toastr.success(data.message);
              break;
            case "requested-items-inventory":
              let optionHtml =
                '<option value="' +
                data.inventory_id +
                '">' +
                data.inventory_name +
                "</option>";
              $("#inventory_id").append(optionHtml);
              $("#inventory_id").val(data.inventory_id).trigger("change");
            ...
            case "mascot-comment":
              let requestData = {
                ...
                default_show_comment: 0,
              };
              $("#comment").val("");
              $.get(
                imageCommentSectionGetUrl,
                requestData,
                function (response) {
                  let commentSection = $("#mascot-image-div-" + data.mascot_id);
                  ...
                  if ($("#mascot-image-div").length) {
                    $("#mascot-image-div").html("");
                    $("#mascot-image-div").append(response);
                    var objDiv = document.getElementById("chat-box");
                    objDiv.scrollTop = objDiv.scrollHeight;
                    $("#comment").val("");
                  }
